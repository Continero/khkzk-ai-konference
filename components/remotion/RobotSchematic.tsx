  ...
  return (
    <AbsoluteFill style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
      <svg width="800" height="520" viewBox="200 40 440 440">
        ...
        {/* Title */}
        <text x="420" y="58" textAnchor="middle" fontFamily="monospace" fontSize="9" fill="rgba(46,196,182,0.4)" letterSpacing="3" opacity={titleOp}>
          HUMANOID_SCHEMATIC // G1_CLASS
        </text>

        {/* Center line guides — appear first */}
        <line x1="420" y1="65" x2="420" y2="470" stroke="rgba(46,196,182,0.04)" strokeWidth="0.5" strokeDasharray="3 6"
        ...
        {parts.map((part, i) => {
          ...
          return (
            <g key={i} opacity={op * pulse} transform={`translate(${tx}, ${ty})`}>
              {/* Glow */}
              <path d={part.d} fill="none" stroke="rgba(46,196,182,0.12)" strokeWidth={4} filter="url(#partGlow)" />
              {/* Main */}
              <path d={part.d} fill={part.fill || "none"} stroke="rgba(46,196,182,0.55)" strokeWidth={1.2} strokeLinejoin="round" strokeLinecap="round" />
            </g>
          );
        })}
        ...
        {frame > 20 && (
          <circle cx="420" cy="190" r={interpolate(frame, [20, 30], [0, 18], { extrapolateRight: "clamp" })}
            fill="none" stroke="rgba(46,196,182,0.2)" strokeWidth={1}
        ...
        {diagnostics.map((diag, i) => {
          ...
          return (
            <g key={`d${i}`} opacity={diagOp}>
              <text x={diag.x} y={diag.y} fontFamily="monospace" fontSize="7" fill="rgba(46,196,182,0.3)" letterSpacing="1" textAnchor={isLeft ? "end" : "start"}>
                {diag.label}
              </text>
              <text x={diag.x} y={diag.y + 12} fontFamily="monospace" fontSize="11" fontWeight="bold" fill="rgba(46,196,182,0.75)" textAnchor={isLeft ? "end" : "start"}>
                {diag.value}
              </text>
              {/* Connecting dot line */}
              <line x1={isLeft ? diag.x + 8 : diag.x - 8} y1={diag.y + 4}
                x2={isLeft ? 370 : 470} y2={diag.y + 4}
                stroke="rgba(46,196,182,0.1)" strokeWidth={0.5} strokeDasharray="2 4" />
            </g>
          );
        })}
        ...
        <g opacity={statusOp}>
          ...
          {/* Progress bar */}
          <rect x="340" y="478" width="160" height="2" rx="1" fill="rgba(46,196,182,0.08)" />
          <rect x="340" y="478" width={160 * statusOp} height="2" rx="1" fill="rgba(46,196,182,0.35)" />
        </g>
      </svg>
    </AbsoluteFill>
  );
